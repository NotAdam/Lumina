...
namespace Lumina.Data.Parsing.Tex.Buffers
{
    ...
    {
        private SquishOptions2 _squishOption = null!;
        ...
        private BlockCompressionTextureBuffer( SquishOptions2 squishOption, TexFile.Attribute attribute, int width, int height,
            ...
            : base( attribute, width, height, depth, mipmapAllocations, buffer, tb => ( (BlockCompressionTextureBuffer) tb )._squishOption = squishOption )
        {
        ...
        public BlockCompressionTextureBuffer( TexFile.TextureFormat format, TexFile.Attribute attribute, int width, int height, int depth,
            ...
            : base( attribute, width, height, depth, mipmapAllocations, buffer, tb => { ( (BlockCompressionTextureBuffer) tb ).SetSquishOption( format ); } )
        {
            SetSquishOption( format );
        }
        ...
        private void SetSquishOption( TexFile.TextureFormat format )
        {
            switch( format )
            {
