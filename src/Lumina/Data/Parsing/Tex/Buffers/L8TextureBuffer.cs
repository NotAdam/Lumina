...
namespace Lumina.Data.Parsing.Tex.Buffers
{
    ...
    {
        /// <inheritdoc />
        public L8TextureBuffer( TexFile.Attribute attribute, int width, int height, int depth, int[] mipmapAllocations, byte[] buffer )
        ...
        protected override TextureBuffer CreateNew( TexFile.Attribute attribute, int width, int height, int depth, int[] mipmapAllocations, byte[] buffer )
            => new L8TextureBuffer( attribute, width, height, depth, mipmapAllocations, buffer );
    }
}