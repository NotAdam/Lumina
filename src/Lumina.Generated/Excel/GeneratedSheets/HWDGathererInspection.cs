...
namespace Lumina.Excel.GeneratedSheets
{
    ...
    public class HWDGathererInspection : IExcelRow
    {
        ...
        public void PopulateData( RowParser parser, Lumina lumina, Language language )
        {
            ...
            for( var i = 0; i < 32; i++ )
                FishParameter[ i ] = new LazyRow< FishParameter >( lumina, parser.ReadColumn< uint >( 32 + i ), language );
            AmountRequired = new byte[ 32 ];
            for( var i = 0; i < 32; i++ )
                AmountRequired[ i ] = parser.ReadColumn< byte >( 64 + i );
            ItemReceived = new LazyRow< Item >[ 32 ];
            for( var i = 0; i < 32; i++ )
                ItemReceived[ i ] = new LazyRow< Item >( lumina, parser.ReadColumn< uint >( 96 + i ), language );
            Reward1 = new LazyRow< HWDGathererInspectionReward >[ 32 ];
            for( var i = 0; i < 32; i++ )
                Reward1[ i ] = new LazyRow< HWDGathererInspectionReward >( lumina, parser.ReadColumn< ushort >( 128 + i ), language );
            Reward2 = new LazyRow< HWDGathererInspectionReward >[ 32 ];
            for( var i = 0; i < 32; i++ )
                Reward2[ i ] = new LazyRow< HWDGathererInspectionReward >( lumina, parser.ReadColumn< ushort >( 160 + i ), language );
            Phase = new LazyRow< HWDGathereInspectTerm >[ 32 ];
            for( var i = 0; i < 32; i++ )
                Phase[ i ] = new LazyRow< HWDGathereInspectTerm >( lumina, parser.ReadColumn< byte >( 192 + i ), language );
        }
    }
}